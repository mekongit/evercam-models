defmodule Camera do
  ...
  @anpr_cameras %{
    "anpr-nezdy"  =>  %{
      anpr_records_url: "wicklow.herbst.ie:11106/ISAPI/Traffic/channels/1/vehicledetect/plates",
      anpr_plate_image_url: "wicklow.herbst.ie:11106/doc/ui/images/plate"
    }
  }

  @required_fields [:name, :owner_id, :config, :is_public, :is_online_email_owner_notification]
  @optional_fields [:exid, :timezone, :thumbnail_url, :status, :offline_reason, :last_polled_at, :alert_emails, :last_online_at, :updated_at, :created_at, :model_id, :location, :location_detailed, :mac_address, :discoverable, :project_id]
  ...
  defp unfinished_projects(query, false), do: query
  defp unfinished_projects(query, true), do: query |> where([cam], cam.status != "project_finished" )

  @doc """
  Returns the list of all unfinished ANPR cameras.
  """
  def get_anpr_cameras() do
    all()
    |> Enum.filter(&anpr_camera?(&1))
    |> Enum.map(fn %{exid: exid} = camera->
      camera |> Map.merge(Map.get(@anpr_cameras, exid))
    end)
  end

  @doc """
  Checks if a camera is an ANPR camera.
  """
  defp anpr_camera?(%{exid: exid}), do:  Enum.member?(Map.keys(@anpr_cameras), exid)
  ...
  def all_geese_cameras() do
    Camera
    |> where([c], like(c.exid, ^"howth%"))
    |> or_where([c], like(c.exid, ^"boote%"))
    |> order_by(asc: :id)
    |> Repo.all
  end
  ...
  def get_rights(camera, user) do
    cond do
      ...
      true ->
        camera.access_rights
        |> Enum.filter(fn(ar) -> Util.deep_get(ar, [:user_id], 0) == user.id && ar.status == 1 end)
